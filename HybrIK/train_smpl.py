...
from hybrik.datasets import MixDataset, PW3D, HP3D, MixDatasetWithPW3D
from hybrik.models import builder
from hybrik.schedulers import schedule_builder
from hybrik.opt import cfg, logger, opt
from hybrik.utils.env import init_dist
from hybrik.utils.metrics import DataLogger, DictLogger, NullWriter, calc_coord_accuracy
...
def train(opt, train_loader, m, criterion, optimizer, scheduler, writer):
    loss_dict_logger = DictLogger()
    ...
    for i, (inps, labels, _, bboxes) in enumerate(train_loader):
        ...
        loss_dict_logger.update(loss_dict, batch_size)
        ...
        if writer is not None and opt.trainIters % opt.logging_period == 0:
            writer.add_scalar(f"train/loss", loss, opt.trainIters)
            writer.add_scalar(f"train/accuvd29", acc_uvd_29_logger.avg, opt.trainIters)
            writer.add_scalar(f"train/acc_xyz_17", acc_xyz_17_logger.avg, opt.trainIters)
            writer.add_scalar(f"train/learning_rate", scheduler.get_last_lr()[0], opt.trainIters)
            
            for k, v in loss_dict_logger.items():
                writer.add_scalar(f"train/{k}", v.avg, opt.trainIters)
            loss_dict_logger.clear()
...
def validate_gt(m, opt, cfg, gt_val_dataset, heatmap_to_coord, batch_size=32, pred_root=False):
    ...
    gt_val_sampler = torch.utils.data.distributed.DistributedSampler(
        gt_val_dataset, num_replicas=opt.world_size, rank=opt.rank) if num_gpu > 1 else None
    ...
    if opt.rank == 0:
        ...
        metrics_17 = gt_val_dataset.evaluate_xyz_17(
            kpt_all_pred, os.path.join(opt.work_dir, 'test_3d_kpt.json'))
        metrics_uvd_24 = gt_val_dataset.evaluate_uvd_24(
            kpt_all_pred, os.path.join(opt.work_dir, 'test_3d_kpt.json'))
        metrics_xyz_24 = gt_val_dataset.evaluate_xyz_24(
            kpt_all_pred, os.path.join(opt.work_dir, 'test_3d_kpt.json'))
        
        metrics_17.update(metrics_uvd_24)
        metrics_17.update(metrics_xyz_24)
...
#         optimizer, milestones=cfg.TRAIN.LR_STEP, gamma=cfg.TRAIN.LR_FACTOR)
    ...
    lr_scheduler = schedule_builder(optimizer, cfg.TRAIN.SCHEDULER)
    ...
    if cfg.DATASET.DATASET == 'mix_smpl':
        train_dataset = MixDataset(
            cfg=cfg,
            train=True)
    elif cfg.DATASET.DATASET == 'mix_smpl_w_pw3d':
        train_dataset = MixDatasetWithPW3D(
            cfg=cfg,
            train=True)
    else:
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(
        train_dataset, num_replicas=opt.world_size, rank=opt.rank) if num_gpu > 1 else None
    ...
    gt_val_dataset_3dhp = HP3D(
        cfg=cfg,
        ann_file='test',
        train=False)

    opt.trainIters = cfg.TRAIN.BEGIN_EPOCH * len(train_loader)
    for _ in range(opt.trainIters):
        lr_scheduler.step()
    ...
    for i in range(cfg.TRAIN.BEGIN_EPOCH, cfg.TRAIN.END_EPOCH):
        ...
        # Training
        loss, acc17 = train(opt, train_loader, m, criterion, optimizer, lr_scheduler, writer)
        logger.epochInfo('Train', opt.epoch, loss, acc17)
        
        with torch.no_grad():
            metrics_h36m = validate_gt(m, opt, cfg, gt_val_dataset_h36m, heatmap_to_coord)
            metrics_3dpw = validate_gt(m, opt, cfg, gt_val_dataset_3dpw, heatmap_to_coord)
            metrics_3dhp = validate_gt(m, opt, cfg, gt_val_dataset_3dhp, heatmap_to_coord)
            gt_tot_err_h36m, gt_tot_err_3dpw, gt_tot_err_3dhp = None, None, None
            if metrics_h36m is not None:
                gt_tot_err_h36m, gt_tot_err_3dpw = metrics_h36m['PA-MPJPE'], metrics_3dpw['PA-MPJPE']
                if writer is not None:
                    writer.add_scalar("train/epoch", opt.epoch, opt.trainIters)
                    for k, v in metrics_h36m.items():
                        writer.add_scalar(f"test_h36m/{k}", v, opt.trainIters)
                    for k, v in metrics_3dpw.items():
                        writer.add_scalar(f"test_3dpw/{k}", v, opt.trainIters)
                    for k, v in metrics_3dhp.items():
                        writer.add_scalar(f"test_3dhp/{k}", v, opt.trainIters)
                
        if (i + 1) % opt.snapshot == 0:
            if opt.log:
                # Save checkpoint
                torch.save(m.module.state_dict() if num_gpu > 1 else m.state_dict(),
                           './exp/{}/{}-{}/model_{}.pth'.format(cfg.DATASET.DATASET, cfg.FILE_NAME, opt.exp_id, opt.epoch))
                # Prediction Test
                if gt_tot_err_h36m <= best_err_h36m:
                    best_err_h36m = gt_tot_err_h36m
                    torch.save(m.module.state_dict() if num_gpu > 1 else m.state_dict(),
                               './exp/{}/{}-{}/best_h36m_model.pth'.format(cfg.DATASET.DATASET, cfg.FILE_NAME, opt.exp_id))
                if gt_tot_err_3dpw <= best_err_3dpw:
                    best_err_3dpw = gt_tot_err_3dpw
                    torch.save(m.module.state_dict() if num_gpu > 1 else m.state_dict(),
                               './exp/{}/{}-{}/best_3dpw_model.pth'.format(cfg.DATASET.DATASET, cfg.FILE_NAME, opt.exp_id))
    ...
    torch.save(m.module.state_dict() if num_gpu > 1 else m.state_dict(), 
               './exp/{}/{}-{}/final_DPG.pth'.format(cfg.DATASET.DATASET, cfg.FILE_NAME, opt.exp_id))
